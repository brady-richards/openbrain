...
4. Carry placeholders forward: the snippet may use the same `{{Placeholder|default}}` convention. After insertion, re-run the placeholder grep on the contract and fill new placeholders. Common snippet placeholders:
   ...
   - `{{Equity Award Shares}}` → **fill with the share count as computed by the equity sheet's G column formula**, which is `=ROUND(F<row> × 'Outstanding Shares'!$B$<n>, -2)` — i.e. rounded to the nearest hundred. Read the value out of G via `sheets_read` with `valueRenderOption=FORMATTED_VALUE`; do not back-of-envelope-multiply, since the unrounded product (e.g. 0.025% × 16,806,522 = 4,201.63) will mismatch the sheet (4,200). The contract already says "Subject to the approval of the Company's Board of Directors..." so writing the proposed number is correct — board approval governs the final grant. Do not leave this blank.
