...
4. Carry placeholders forward: the snippet may use the same `{{Placeholder|default}}` convention. After insertion, re-run the placeholder grep on the contract and fill new placeholders. Common snippet placeholders:
   ...
   - `{{Equity Award Shares}}` → **read the value out of the equity sheet's G column for the new hire's row** (`sheets_read` with `valueRenderOption=FORMATTED_VALUE`). The sheet is the source of truth; don't recompute from the % and outstanding shares, because the G formula may apply rounding (currently `ROUND(..., -2)`) or other adjustments that a naive multiplication won't match, and the formula can change over time. The contract already says "Subject to the approval of the Company's Board of Directors..." so writing the proposed number is correct — board approval governs the final grant. Do not leave this blank.
