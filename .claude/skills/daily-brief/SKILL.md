...
2. **Priority mail.** For each `gmail_*` MCP, `gmail_search_messages` with `is:unread newer_than:2d (is:important OR is:starred OR label:^iim)`. Cap at 5 per account. Capture subject, sender, account slug. **Before surfacing an item as "needs a reply"**, apply two filters: (a) **already handled** — drop if a completed Asana task references the same Gmail thread ID or closely matches the subject; (b) **distribution list traffic** — read the email and drop it if the only recipient is a list Brady is on (e.g. `guides@doromind.com`) and Brady is not mentioned by name in the subject or body.
