...
6b. **Draft replies for actionable threads.** After steps 1–6, draft responses for "Needs a reply" items where the user is the next actor. Skip:
   ...
   3. **Compose draft.** Match the user's voice (see CLAUDE.md §6). Lead with the ask or the answer. For email: use `Re: <original subject>`. For Slack: no subject. **Formatting: write each paragraph as a single unbroken line. Only use blank lines (`\n\n`) between paragraphs. Never insert `\n` within a paragraph — Gmail preserves hard line breaks and renders them as a narrow column instead of reflowing.**
   4. **Save draft.**
