...
2. **Priority mail.** For each `gmail_*` MCP, `gmail_search_messages` with `is:unread newer_than:2d (is:important OR is:starred OR label:^iim)`. Cap at 5 per account. Capture subject, sender, thread ID, account slug. **Before surfacing an item as "needs a reply"**, apply two filters: (a) **already handled** — drop if a completed Asana task has `Gmail Thread Id` (custom field `1213297635072824`) matching the Gmail thread ID, or closely matches the subject; (b) **distribution list traffic** — read the email and drop it if the only recipient is a list Brady is on (e.g. `guides@doromind.com`) and Brady is not mentioned by name in the subject or body.
3. **Slack attention.** For each `slack_*` MCP, search for mentions of the user in the last 24h and list unread DMs.
3. **Message attention.** For each mac messages MCP, search for mentions of the user in the last 24h and list unread messages
4. **Overdue tasks.** 
    - For both `asana_personal` and `asana_work`, `asana_get_my_tasks` filtered to `completed_since=now` and due date < today. Use `opt_fields: name,due_on,completed,assignee_section.name,permalink_url,custom_fields.gid,custom_fields.name,custom_fields.display_value`. Custom field GIDs for the work workspace: `1207543199556043` Status (Global), `1213297635072824` Gmail Thread Id, `1213390228368672` Hours Remaining, `1213504813625904` External due date, `1211397212267647` Fathom Meeting Title, `1211397376866871` Fathom Link, `1212969581024580` State, `1213014460079617` Responsible Unit. Apply Status (Global) filtering before surfacing tasks: exclude **Done** and **Backlog** from overdue lists; show **Blocked** tasks as "⏸ check in" not urgent; suppress **Scheduled** tasks until their due date; **In Progress** = actively being worked, don't over-flag; **To Do** = normal treatment.
    - For mac reminders, do the same
...
7. **Compose the daily note.** If `+ Atlas/Daily/<date>.md` does not exist, scaffold from `+ Extras/Templates/Daily.md`. If a `## Morning brief` section already exists in the note, **replace its body in place** (find the `## Morning brief` heading and overwrite everything up to the next H2 or EOF). Otherwise insert a new `## Morning brief` section near the top. Contents:
   ...
   - **Needs a reply** (mail + slack + messages, grouped by account/workspace)
   - **Overdue in Asana**
