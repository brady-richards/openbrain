...
2. **Priority mail.** For each `gmail_*` MCP, `gmail_search_messages` with `is:unread newer_than:2d (is:important OR is:starred OR label:^iim)`. Cap at 5 per account. Capture subject, sender, account slug. **Before surfacing an item as "needs a reply", check whether it is already handled:** a mail item is considered done if (a) there is a completed Asana task whose title or description references the same Gmail thread ID, or (b) a completed Asana task title closely matches the email subject. Drop handled items silently — do not list them in the brief.
