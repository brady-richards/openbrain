...
- **Home.md** is the front door; its "Top MOCs" section is auto-regenerated by the Stop hook.
...
- Git is the sole sync mechanism (Obsidian Sync is disabled). All cross-device sync goes through the stop hook's commit-and-push cycle.
...
- **Never use deprecated remote connectors** — `mcp__claude_ai_Asana__*`, `mcp__claude_ai_Google_Calendar__*`, `mcp__gmail__*`, `mcp__claude_ai_Slack__*` are single-account and bypass the multi-account routing defined in §12. Use the local `asana_*` / `gmail_*` / `gcal_*` / `gmeet_*` / `gdrive_*` / `slack_*` MCPs instead.
...
Two git/Claude-Code hooks handle maintenance automatically:

- **Stop hook** (`.openbrain/on-stop.sh`) — regenerates `Home.md` MOC index, then smart-commits and pushes (skip-if-clean, pull-rebase first, conflict → inbox note).
- **SessionStart hook** (`.openbrain/on-start.sh`) — `git pull --rebase` (fail-soft; never blocks).
- **Pre-commit hook** (`.openbrain/pre-commit.sh`) — frontmatter + broken-link linter, warn-only.
...
Gmail, Calendar, Meet, and Drive/Docs/Sheets are separate MCP packages (`@gongrzhe/server-gmail-autoauth-mcp`, `@cocal/google-calendar-mcp`, `@dtannen/google-meet-mcp`, and `@a-bonus/google-docs-mcp`), each launched per Google account — so each Google identity gets a `gmail_*`, `gcal_*`, `gmeet_*`, and `gdrive_*` server.
...
- `#gmail/<slug>`, `#gcal/<slug>`, `#gmeet/<slug>`, `#gdrive/<slug>` — one per Google account
...
**Generated by the [openbrain-template](https://github.com/davidianstyle/openbrain-template) bootstrap on `2026-04-13`.** Re-run `bootstrap/setup.sh` to update this file after adding new services.
