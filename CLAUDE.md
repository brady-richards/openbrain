...
This vault is **brichards's OpenBrain**: a personal LYT (Linking Your Thinking) knowledge base where Claude Code is a first-class collaborator. Read this file at the start of every session in this vault.

> This file was generated from the [openbrain-claude-starter](https://github.com/davidianstyle/openbrain-claude-starter) template (formerly **openbrain-template**). Re-run `bootstrap/setup.sh` to regenerate account tables after adding/removing services, or run `bootstrap/lib/add-*.sh` for incremental changes.

## 1. Identity and structure

- **Primary collaborator:** brichards.
...
- **Pre-commit hook** (`.openbrain/pre-commit.sh`) — frontmatter + broken-link linter, warn-only. Linked by `bootstrap/setup.sh` into `.git/hooks/pre-commit`.
- **Auto git sync hooks** (not enabled by default — opt in during `./bootstrap/setup.sh` or by adding the hooks section to `.claude/settings.json`):
...
| `/pull-openbrain-template` | Pull latest infrastructure changes from the upstream openbrain-claude-starter repo (same slash family as `/pull-openbrain-claude-starter`). Skill file retains the `-template` path for compatibility. |
| `/push-openbrain-template` | Genericize vault improvements and open a PR against openbrain-claude-starter — same upstream as `/push-openbrain-claude-starter`; skill folder name is legacy. |
...
**Generated by the [openbrain-claude-starter](https://github.com/davidianstyle/openbrain-claude-starter) bootstrap on `2026-04-13`.** Re-run `bootstrap/setup.sh` to update this file after adding new services.
